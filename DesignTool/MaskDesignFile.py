...
class MaskDesignFile:
    ...
    def genMaskDesign(self):    
        ...
        params = tlist.config.properties['params'].properties
        cols = []
    ...
    def genMaskBlu (self):
        tlist = self.targetList
        targets = tlist.targets
        params = tlist.config.get('params')
        cols = []
        ts = datetime.datetime.strptime(params.get('ObsDate')[0], '%Y-%m-%d %H:%M:%S')
        obsDate = ts.strftime ('%Y-%m-%d')
        obsTime = sexg2Float(ts.strftime ('%H:%M:%S')) * 15  # to degree
        refWave = float(params.get('CenterWaveLength')[0]) / 10  # to nanometer
        
        cols.append(pf.Column (name='BluId', format='I11', null='-9999', unit='None', array=[1]))
        cols.append(pf.Column (name='DesId', format='I11', null='-9999', unit='None', array=[1]))
        cols.append(pf.Column (name='BluName', format='A68', null='INDEF', unit='None', array=[params.get('MaskName')[0]]))
        cols.append(pf.Column (name='guiname', format='A8', null='INDEF', unit='None', array=[params.get('MaskName')[0]]))
        cols.append(pf.Column (name='BluObsvr', format='A68', null='INDEF', unit='None', array=[params.get('Observer')[0]]))
        ...
        cols.append(pf.Column (name='TELESCOP', format='A68', null='INDEF', unit='None', array=[params.get('Telescope')[0]]))
        cols.append(pf.Column (name='RefrAlg', format='A68', null='INDEF', unit='None', array=['N/A']))
    
        cols.append(pf.Column (name='AtmTempC', format='F5.1', null='-9999', unit='degC', array=[params.get('Temperature')[0]]))
    
        cols.append(pf.Column (name='AtmPres', format='F6.1', null='-999.0', unit='hPa', array=[params.get('Pressure')[0]]))
    ...
    def genBluSlits (self):
        targets = self.targetList.targets
        params = self.targetList.config.get('params')
        ...
        relAngles = np.radians(float(params.get('MaskPA')[0]) - objInMask.slitPA)
...
if __name__ == "__main__":    
    config = ConfigFile('./smdt.cfg', split=False)
    params = ConfigFile(config.get('paramFile'), split=True)
    config.properties['params'] = params
    tlist = TargetList('testData/CetusIII.lst', False, config)
    
    print ("Checking which targets are inside the mask")
    inOutChecker = InOutChecker(MaskLayouts[params.get('Instrument')[0].lower()])
    ...
    fileout = "maskDesignFileTest.fits" 
